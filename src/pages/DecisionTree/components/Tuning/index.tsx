...
import { message, Upload, Select, Form, Button } from "antd";
...
const Tuning: React.FC<DecisionTreePropsType> = ({ next }) => {
  ...
  const onFinish = async (value: LRTRainPropsType) => {
    setSubmiting(true);
    try {
      await LinearRegressionTrain(Object.assign({}, { ...value, md5: MD5 }));
    } catch (error: any) {
      // message.error(error);
    }
    setSubmiting(false);
    ...
    async onChange(info) {
      ...
      if (status === "done") {
        ...
        try {
          ...
          console.log(`${info.file.name} Upload Response: ${response}`);
          setMD5(md5);
    ...
    <Fragment>
      ...
      {allFeatures.length > 0 && (
        <Form
          ...
          initialValues={{ƒfeatures: allFeatures }}
          onFinish={onFinish}
          autoComplete="off"
        >
