...
// 枚举常量
const NAV_ITEM_LABELS = ['Home Page', 'Machine Learning'];
...
// 初始化顶部导航数据结构
const navItems: MenuProps['items'] = NAV_ITEM_LABELS.map(
  (key, index) => ({
    key: `nav${index}`,
...
const App: React.FC = () => {
  ...
  const [siderSelectedKeys, setSiderSelectedKeys] =
    useState(['sider0-0']);

  //切换侧边导航
  function changeSiderSelectedKey(item: any) {
    console.log('切换侧边导航', item);
    if (item.keyPath.length < 2) return;
    setSiderSelectedKeys([item.key]);
  }

  // 切换顶部导航
  function changeNavSelectedKey(item: {
    key: string;
    keyPath: string[];
  }) {
  ...
  return (
    <Layout style={{ maxHeight: '100vh' }}>
      ...
      >
            ...
            items: [
              {
                key: '1',
                label: (
                  <div onClick={exitLogin}>Logout</div>
                ),
              },
            ],
          }}
        ...
        >
          <Button type="primary">
            <div>{`Welcome ${localStorage.getItem(
              'username'
            )}`}</div>
          </Button>
        </Dropdown>
      ...
      <Layout>
        ...
        >
          ...
          >
            {(function () {
              try {
                const [siderIndex, itemIndex] =
                  parseSubString(siderSelectedKeys[0]);
                if (
                  CONTENT_PAGES[siderIndex] &&
                  CONTENT_PAGES[siderIndex][itemIndex]
                )
                  return CONTENT_PAGES[siderIndex][
                    itemIndex
                  ];
